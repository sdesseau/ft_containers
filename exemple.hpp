...
namespace ft
{
	...
	{   
		public:
			...
			*/
			size_type   size(void) const { return (this->_size - this->_begin); }
			...
			** (An unsigned integral type)
			*/
			size_type   capacity (void) const { return (this->_capacity - this->_begin); }
			...
			*/
			void        reserve (size_type n)
			{
				...
				else if (n > this->capacity())
				{
					...
					_capacity = _begin + n;
					_size = _begin;
					while (prev_begin != prev_size)
					{
						_alloc.construct(_size, *prev_begin);
						_size++;
						prev_begin++;
					}
			...
			*/
			template <class InputIterator>
				...
				typename ft::enable_if<!ft::is_integral<InputIterator>::value, InputIterator>::type* = u_nullptr)
				{
					...
					if (size_type(_capacity - _begin) >= dist)
					{
					...
					else
					{
						...
						new_size = new_begin;
						new_capacity = new_begin + dist;

						for(; &(*first) != &(*last); first++, new_size++)
							_alloc.construct(new_size, *first);
			...
			*/
			void assign (size_type n, const value_type& val)
			{
				...
				if (n == 0)
					return ;
				if (size_type(_capacity - _begin) >= n)
				{
					while (n)
					{
						_alloc.construct(_size++ , val);
			...
			iterator insert (iterator position, const value_type& val)
			{
				...
				if (size_type(_capacity - _size) >= this->size() + 1)
				{
					for (size_type i = 0; i < pos_len; i++)
						_alloc.construct(_size - i, *(_size - i - 1));
					_size++;
				...
				else
				{
					...
					new_begin = _alloc.allocate( next_capacity );
					new_size = new_begin + this->size() + 1;
					new_capacity = new_begin + next_capacity;

					for (size_type i = 0; i < pos_len; i++)
						_alloc.construct(new_begin + i, *(_begin + i));
					_alloc.construct(new_begin + pos_len, val);
					for (size_type j = 0; j < this->size() - pos_len; j++)
						_alloc.construct(new_size - j - 1, *(_size - j - 1));
			...
			*/
			void insert (iterator position, size_type n, const value_type& val)
			{
				...
				else
				{
					...
					if (size_type(new_capacity - new_begin) < this->size() + n)
					{
						if (new_begin)
							_alloc.deallocate(new_begin, new_begin - new_capacity);
						next_capacity = this->size() + n;
						new_begin = _alloc.allocate(next_capacity);
						new_size = new_begin + this->size() + n;
						new_capacity = new_begin + next_capacity;
					}
					...
					for (size_type k = 0; k < n; k++)
						_alloc.construct(new_begin + pos_len + k, val);
					for (size_type j = 0; j < (this->size() - pos_len); j++)
						_alloc.construct(new_size - j - 1, *(_size - j - 1));
			...
			*/
				...
				typename ft::enable_if<!ft::is_integral<InputIterator>::value, InputIterator>::type* = u_nullptr)
			{
				...
				else
				{
					...
					new_size = new_begin + this->size() + dist;
					new_capacity = new_begin + ( this->size() * 2 );

					if (size_type(new_capacity - new_begin) < this->size() + dist)
					{
						if (new_begin)
							_alloc.deallocate(new_begin, new_capacity - new_begin);
					...
					for (size_type k = 0; k < this->size() - (&(*position) - _begin); k++)
						_alloc.construct(new_begin + (&(*position) - _begin) + dist + k, *(_begin + (&(*position) - _begin) + k));
			...
			*/
			iterator erase (iterator position)
			{
				...
				{
					for (int i = 0; i < _size - &(*position) - 1; i++)
					{
			...
			*/
			void swap (vector& x)
			{
				...
				pointer save_begin = x._begin;
				pointer save_size = x._size;
				pointer save_capacity = x._capacity;
